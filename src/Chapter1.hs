...
sumLast2 n = penultimate_digit n + last_digit n
  where
    penultimate_digit :: Integral a => a -> a
    penultimate_digit x = last_digit $ div (abs x) 10
    last_digit :: Integral a => a -> a
    last_digit x = mod (abs x) 10
...
firstDigit 0 = 0
firstDigit n =
  let divided = div (abs n) 10 in if divided > 0 then firstDigit divided else n
